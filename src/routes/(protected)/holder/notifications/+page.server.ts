...
type Notification = {
	id: string;
	type: 'mapping_approved' | 'mapping_rejected' | 'profile_verified' | 'account_parked' | 'account_unmapped' | 'system';
	title: string;
	message: string;
	metadata: any;
	readAt: string | null;
...
export const load: PageServerLoad = async ({ parent, cookies, url }) => {
	...
	// Fetch notifications for this holder
	let query = (supabase
		.from('notifications')
	...
	const { data: allNotificationsData } = await ((supabase
		.from('notifications')
		.select('type, read_at') as any)
		.eq('user_id', user.id));

	// Calculate statistics
	const total = allNotificationsData?.length || 0;
	const unread = allNotificationsData?.filter((n: any) => !n.read_at).length || 0;
	const approvals = allNotificationsData?.filter((n: any) => n.type === 'mapping_approved').length || 0;
	const rejections = allNotificationsData?.filter((n: any) => n.type === 'mapping_rejected').length || 0;
	const system = allNotificationsData?.filter((n: any) => n.type === 'system' || n.type === 'profile_verified' || n.type === 'account_parked' || n.type === 'account_unmapped').length || 0;
